...
// Finds TGExtra.bundle using dladdr (works on both jailbroken AND sideloaded installs).
...
- (NSString *)findBundlePath {
	...
	IMP imp = class_getMethodImplementation(
		object_getClass([TGExtraLocalization class]),
		@selector(shared)
	);
	...
	if (imp && dladdr((const void *)imp, &info) && info.dli_fname) {
		...
		// 1. Next to the dylib (Frameworks/TGExtra.bundle)
		NSString *candidate = [dylibDir stringByAppendingPathComponent:@"TGExtra.bundle"];
		...
		candidate = [[dylibDir stringByDeletingLastPathComponent]
		             stringByAppendingPathComponent:@"TGExtra.bundle"];
		if ([[NSFileManager defaultManager] fileExistsAtPath:candidate]) return candidate;
	...
	NSString *jbPath = [NSString stringWithFormat:@"%@/TGExtra.bundle",
...
- (void)loadDefault {
	NSString *lang = [[NSUserDefaults standardUserDefaults]
	                  stringForKey:@"TGExtraLanguage"] ?: @"en";
...
+ (NSString *)localizedStringForKey:(NSString *)key {
	...
	NSString *result = [TGExtraLocalization shared].strings[key];
	if (result) return result;

	// Fallback to TGLocalization (for languages loaded via that path)
	result = [[TGExtraLocalization shared].localization get:key];
